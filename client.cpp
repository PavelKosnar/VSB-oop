...
Client::~Client() {
    ...
    if (mailingAddresses != nullptr) {
        for (int i = 0; i < mailingAddressesCount; i++) {
            delete mailingAddresses[i];
        }
        delete[] mailingAddresses;
    }

    delete creditCard;

    address = nullptr;
    mailingAddresses = nullptr;
    creditCard = nullptr;
    mailingAddressesCount = 0;
}
